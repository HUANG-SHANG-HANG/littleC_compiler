...
/* ============================================================
 *        驱动模块 (main)
 *        读源文件 → 词法分析 → 语法分析 → 输出 AST
 * ============================================================ */
#ifdef PARSER_STANDALONE 
int main(int argc, char* argv[]) {
    std::string inFile  = "source.txt";
    std::string outFile = "parse_out.txt";
    ...
    if (parser.hasError()) {
        ...
        fout << "Parsing OK.\n\n";
        fout << "=== Abstract Syntax Tree ===\n\n";
        fout << Parser::astToString(ast);
    }
    fout.close();


    // 5. 控制台输出
    std::cout << "Done!  Result -> " << outFile << std::endl;
    if (lexer.hasError())  std::cout << "[Lexer Errors]\n"  << lexer.getErrors();
    if (parser.hasError()) std::cout << "[Parser Errors]\n" << parser.getErrors();
    if (!parser.hasError()) {
        std::cout << "\n" << Parser::astToString(ast);
    }
